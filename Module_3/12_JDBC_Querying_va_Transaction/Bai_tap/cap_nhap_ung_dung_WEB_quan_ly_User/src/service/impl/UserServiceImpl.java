...
public class UserServiceImpl implements UserService {
    ...
    public List<User> findAllSP() {
        return this.repository.findAllSP();
    }

    @Override
    public boolean updateSP(int id, String name, String email, String country) {
        if (!name.matches(regexName)){
            return false;
        }
        if (!email.matches(regexEmail)){
        ...
        return repository.updateSP(id, new User(name, email, country));
    }

    @Override
    public boolean removeSP(int id) {
        return this.repository.removeSP(id);
    }

    @Override
    public boolean addUserTransaction(String name, String email, String country, int[] permisions) {
        if (!name.matches(this.regexName)){
            return false;
        }
        if (!email.matches(this.regexEmail)){
        ...
        return repository.addUserTransaction(new User(name, email, country) , permisions);
    }
}
