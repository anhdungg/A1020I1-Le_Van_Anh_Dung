...
public class BlogController {
    ...
    @GetMapping(value = "/")
    public String home(@PageableDefault(1) Pageable pageable, Model model, HttpServletRequest request, @ModelAttribute("admin") User admin){
        if ((admin.getPassword() != null) && (admin.getUserName() != null)){
            this.role = "admin";
        }else {
            User user = this.checkAdmin(request);
            if (user != null){
                this.role = "admin";
        ...
        if (cookies != null){
            for (Cookie ck : cookies){
                if (ck.getName().equals("user")){
                    if (ck.getValue().equals("ledung.ht17@gmail.com")){
                        user.setUserName(ck.getValue());
                    }
                }
                if (ck.getName().equals("pass")){
                    if (ck.getValue().equals("LeDung123")){
